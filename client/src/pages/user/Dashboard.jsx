...
const layananList = [{ jenis: 'tidak_mampu', label: 'Ket. Tidak Mampu', desc: 'Keterangan ekonomi' }];
...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-(--bg) text-(--text) font-sans pb-24 md:pb-8 transition-colors duration-300">
      ...
      <div className="px-5 md:px-0 grid grid-cols-1 md:grid-cols-12 gap-8">
        <div className="md:col-span-8 space-y-6 md:space-y-8">
          ...
          <Link
            to="/layanan/ajukan?jenis=tidak_mampu"
            className="block relative bg-linear-to-br from-emerald-500 to-teal-600 border border-emerald-400 rounded-3xl p-6 md:p-8 overflow-hidden group shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/30 transition-all hover:-translate-y-1"
          >
          ...
          <div className="grid grid-cols-1 gap-6 md:gap-8 min-h-75 md:min-h-85">
        ...
        <div className="md:col-span-4 space-y-6 md:space-y-8">
          <section className="bg-(--bg-raised) border border-(--border) rounded-3xl p-5 md:p-8 h-full flex flex-col relative overflow-hidden group">
            ...
            {loadingRiwayat ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-6 text-center border border-dashed border-(--border) rounded-2xl bg-(--bg-overlay)">
                <div className="w-10 h-10 bg-(--bg-raised) border border-(--border) rounded-full flex items-center justify-center mb-3 shadow-sm">
                  <FileText className="w-5 h-5 text-(--text-muted)" />
                </div>
                <p className="text-xs text-(--text-muted) font-medium">Belum ada pengajuan surat</p>
              </div>
            ) : (
              <div className="space-y-3 flex-1 relative z-10">
                {riwayat.map((r) => {
                  ...
                  return (
                    <div key={r._id} className="group flex flex-col p-4 bg-(--bg-overlay) border border-(--border) rounded-2xl hover:border-(--border-hover) transition-colors relative overflow-hidden">
                      {/* Garis warna di pinggir buat indikator status */}
                      <div className={`absolute left-0 top-0 bottom-0 w-1 ${statusColor[r.status]?.replace('text-', 'bg-') || 'bg-(--border)'}`} />

                      <div className="flex items-start justify-between gap-3 font-medium mb-1.5 pl-2">
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold truncate text-(--text) transition-colors">{layanan?.label || r.jenisSurat}</p>
                        </div>
                        <div className={`text-xs font-black uppercase tracking-wider px-2 py-0.5 rounded-md bg-(--bg-raised) border border-(--border) ${statusColor[r.status]}`}>{statusLabel[r.status]}</div>
                      </div>
                      <div className="flex items-center gap-2 mt-1 pl-2">
                        <p className="text-xs text-(--text-muted) font-medium tracking-wide">{new Date(r.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                      </div>
                    </div>
                  );
                })}
