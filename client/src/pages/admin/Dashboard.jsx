...
import { FileText, ChevronRight, LogOut, CheckCircle, XCircle, Settings, ClipboardList } from 'lucide-react';
...
const StatCard = ({ icon, label, value, color }) => {
  ...
  return (
    <div className="p-4 rounded-xl glass-panel flex items-center gap-4 hover:border-blue-500/20 transition-colors">
      <div className="w-12 h-12 rounded-xl bg-(--card-bg) flex items-center justify-center shrink-0">
    ...
    <div className="space-y-8 animate-in fade-in duration-500 px-5 md:px-0 pt-8 md:pt-0 pb-24 md:pb-0">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="space-y-8">
          ...
          <div className="p-6 glass-card rounded-4xl w-full shadow-sm">
            <div className="flex items-center justify-between mb-6 border-b border-(--card-border) pb-4">
              <h2 className="text-sm font-bold text-(--text)">Pengajuan Terbaru</h2>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <div className="w-12 h-12 bg-white/2 rounded-full flex items-center justify-center mb-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {recentPengajuan.map((p) => (
                  <div key={p._id} className="group flex items-center gap-3 p-4 glass-panel rounded-2xl transition-all cursor-default">
                    <div className="w-10 h-10 rounded-xl bg-(--card-bg) flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                      <LetterIcon jenis={p.jenisSurat} className="w-5 h-5 text-(--text-muted)" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold truncate text-(--text) group-hover:text-(--text) transition-colors">{p.namaLengkap}</p>
                      <p className="text-[10px] text-(--text-muted) font-bold uppercase mt-0.5 tracking-tighter">{new Date(p.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                    </div>
                    <div className={`text-[10px] font-black uppercase tracking-tighter px-2 flex items-center h-6 bg-(--card-bg) rounded border border-(--card-border) ${statusColor[p.status]}`}>{p.status}</div>
                  </div>
                ))}
              </div>
            )}
