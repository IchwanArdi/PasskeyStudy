...
import { ArrowLeft, ChevronDown, ChevronUp, Check, X, RefreshCw, Filter, DownloadCloud, Trash2 } from 'lucide-react';
...
const AdminPengajuan = () => {
  ...
  const fetchPengajuan = useCallback(async () => {
    setLoading(true);
    try {
      const url = filterStatus ? `/pengajuan/admin/semua?status=${filterStatus}` : `/pengajuan/admin/semua`;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <header className="flex items-center justify-between">
          <div>
            ...
            <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight bg-linear-to-r from-(--heading-from) to-(--heading-to) bg-clip-text text-transparent">Kelola Pengajuan Surat</h1>
          </div>
          <button
            onClick={fetchPengajuan}
            title="Refresh Data"
            className="p-2.5 md:p-3 bg-(--card-bg) border border-(--card-border) rounded-2xl text-(--text-muted) hover:text-(--text) transition-all flex items-center justify-center shadow-sm"
          >
        ...
        <div className="relative w-full max-w-xs md:max-w-sm ml-1">
          ...
          <select
            ...
            className="block w-full pl-10 pr-10 py-3 text-xs font-semibold tracking-widest rounded-2xl appearance-none glass-panel text-(--text) focus:outline-none focus:border-blue-500/40 transition-all cursor-pointer shadow-sm"
          >
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="py-24 text-center bg-(--card-bg) border border-dashed border-(--card-border) rounded-[2.5rem]">
              <p className="text-(--text-muted) text-xs font-bold uppercase tracking-widest">Kosong nih, gak ada pengajuan.</p>
            </div>
          ) : (
            pengajuan.map((p) => {
              ...
              return (
                <div key={p._id} className={`glass-card border transition-all rounded-[28px] overflow-hidden ${isOpen ? 'border-blue-500/40' : 'hover:border-white/10'}`}>
                  <button onClick={() => setExpanded(isOpen ? null : p._id)} className="w-full flex items-center gap-3 md:gap-4 p-4 md:p-6 text-left">
                    <div className="w-12 h-12 md:w-16 md:h-16 bg-(--card-bg) rounded-2xl flex items-center justify-center shrink-0 border border-(--card-border)">
                      <LetterIcon jenis={p.jenisSurat} className="w-6 h-6 md:w-8 md:h-8 text-blue-400/80" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm md:text-lg font-black truncate text-(--text) tracking-tight">{p.namaLengkap}</p>
                      <p className="text-[10px] md:text-xs text-(--text-muted) font-bold uppercase tracking-widest mt-0.5">
                    ...
                    <div className="w-8 h-8 rounded-full bg-(--card-bg) flex items-center justify-center ml-2 shrink-0 border border-(--card-border)">
                  ...
                  {isOpen && (
                    <div className="px-6 pb-6 border-t border-white/3 pt-6 animate-in slide-in-from-top-2 duration-300">
                        ...
                        <div className="space-y-4 glass-panel p-5 rounded-3xl">
                          {[
                            ...
                            <div key={item.l} className="flex flex-col gap-1">
                              <span className="text-[9px] font-black text-(--text-muted) tracking-tighter">{item.l}</span>
                              <span className="text-xs md:text-sm font-bold text-(--text)">{item.v}</span>
                            </div>
                          ))}
                        ...
                        <div className="space-y-6">
                          <div className="space-y-3">
                            ...
                            <textarea
                              ...
                              rows={4}
                              className="w-full px-5 py-4 bg-(--card-bg) border border-(--card-border) rounded-3xl text-sm focus:outline-none focus:border-blue-500/40 transition-all resize-none placeholder:text-(--text-muted) text-(--text)"
                            />
                          </div>
                          ...
                          {(p.status === 'disetujui' || p.status === 'ditolak') && (
                            <div className="space-y-4">
                              ...
                              {p.status === 'disetujui' && (
                                ...
                                >
                                  {downloadingId === p._id ? <div className="w-4 h-4 border-2 border-blue-400/30 border-t-blue-400 rounded-full animate-spin" /> : <DownloadCloud className="w-5 h-5" />}
