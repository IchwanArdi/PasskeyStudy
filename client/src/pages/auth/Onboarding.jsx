...
const Onboarding = () => {
  ...
  return (
    <div className="min-h-screen bg-(--bg) text-(--text) font-sans flex flex-col overflow-hidden transition-colors duration-300">
      {/* Tombol Lewati (Skip) */}
      <div className="flex justify-end px-6 pt-6 relative z-20">
        <button onClick={finish} className="text-xs text-gray-600 hover:text-gray-400 transition-colors font-medium tracking-wide">
          Lewati
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 py-6 text-center relative">
        ...
        <div className={`absolute inset-0 pointer-events-none flex items-center justify-center opacity-30`}>
          <div className={`w-96 h-96 rounded-full bg-gradient-radial blur-3xl bg-linear-to-br ${slide.color}`} />
        </div>

        {/* Ikon Slide */}
        <div className={`relative z-10 w-24 h-24 rounded-3xl bg-linear-to-br ${slide.color} border ${slide.border} flex items-center justify-center mb-8 shadow-2xl`}>
          <slide.icon className={`w-12 h-12 ${slide.accent}`} />
        </div>

        {/* Baris Teks Slide */}
        <div className="relative z-10 max-w-sm">
          {slide.desa && <p className={`text-xs font-bold uppercase tracking-widest ${slide.accent} mb-2`}>{slide.desa}</p>}
          <h1 className="text-3xl font-black tracking-tight mb-2 leading-tight">{slide.title}</h1>
          <p className="text-xl font-bold text-(--text-muted) mb-6">{slide.subtitle}</p>
          <p className="text-(--text-muted) text-sm leading-relaxed mb-8">{slide.desc}</p>

          {/* Daftar Fitur (hanya muncul di slide tertentu) */}
          {slide.features && (
            <div className="flex flex-wrap gap-2 justify-center mb-4">
              {slide.features.map((f) => (
                <span key={f} className={`px-3 py-1 rounded-full text-[10px] md:text-xs font-semibold bg-linear-to-br ${slide.color} border ${slide.border} ${slide.accent}`}>
                  {f}
                </span>
              ))}
            </div>
      ...
      <div className="px-8 pb-10 flex flex-col items-center gap-6 relative z-20">
        ...
        <div className="flex gap-2">
          {slides.map((_, i) => (
            <button key={i} onClick={() => setCurrent(i)} className={`h-1.5 rounded-full transition-all duration-300 ${i === current ? `w-8 ${slide.accent.replace('text-', 'bg-')}` : 'w-1.5 bg-(--card-border)'}`} />
          ))}
        </div>
        ...
        <button
          ...
          className={`w-full max-w-sm py-4 rounded-2xl font-bold text-base flex items-center justify-center gap-3 transition-all active:scale-95 shadow-xl ${
            isLast ? 'bg-(--text) text-(--bg) hover:opacity-90' : 'bg-(--card-bg) border border-(--card-border) text-(--text) hover:opacity-80'
          }`}
        >
          {isLast ? (
            <>
