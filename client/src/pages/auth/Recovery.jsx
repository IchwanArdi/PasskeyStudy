...
const Recovery = () => {
  ...
  return (
    <div className="min-h-screen bg-(--bg) text-(--text) font-sans flex items-center justify-center p-6 transition-colors duration-300 relative overflow-hidden">
      <div className="relative z-10 w-full max-w-sm animate-fade-in-up">
        {/* Indikator Langkah (Stepper) */}
        <div className="flex items-center justify-center gap-3 mb-10">
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center gap-3">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${step >= s ? 'bg-emerald-500 text-black' : 'bg-white/4 text-gray-600 border border-white/8'}`}>
                {step > s ? <Check className="w-3 h-3" /> : s}
              </div>
              {s < 3 && <div className={`w-12 h-0.5 rounded-full ${step > s ? 'bg-emerald-500' : 'bg-white/6'}`} />}
            </div>
          ))}
        </div>
              ...
              <form onSubmit={handleVerifyCode} className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full px-4 py-3 bg-white/3 border border-white/8 rounded-xl text-white placeholder-gray-600 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 transition-all text-sm"
                ...
                {error && <div className="px-4 py-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-xs font-medium italic">{error}</div>}
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-4 bg-emerald-500 text-black font-bold rounded-xl hover:bg-emerald-400 transition-all text-sm flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 active:scale-95 disabled:opacity-50"
                >
        ...
        {step === 2 && (
          <div>
            ...
            <div className="glass-card rounded-3xl p-6 sm:p-8 space-y-6">
              {message && <div className="px-4 py-3 bg-emerald-500/6 border border-emerald-500/15 rounded-xl text-emerald-400 text-xs text-center font-medium animate-pulse">{message}</div>}
              {error && <div className="px-4 py-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-xs font-medium italic">{error}</div>}
              <button
          ...
          <div>
            ...
            {newRecoveryCodes.length > 0 && (
              <div className="glass-card rounded-3xl p-6 sm:p-8 mb-6">
                <div className="p-3.5 bg-yellow-500/10 border border-yellow-500/20 rounded-xl mb-5 flex gap-3">
                  <AlertTriangle className="w-5 h-5 text-yellow-400 shrink-0" />
                  <div className="flex-1 text-left">
                    <p className="text-[10px] text-yellow-400 font-bold mb-1 uppercase tracking-wider">KODE PEMULIHAN BARU</p>
                    <p className="text-[11px] text-gray-500 leading-relaxed font-medium">Kode lama Anda sudah hangus. Simpan kode-kode di bawah ini di tempat aman.</p>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-2 mb-5">
                  {newRecoveryCodes.map((c, i) => (
                    <div key={i} className="p-3 bg-(--card-bg) border border-(--card-border) rounded-lg text-center font-mono text-xs tracking-widest text-(--text)">
                      {c}
                    </div>
                  ))}
                </div>
                ...
                <div className="flex gap-2 mb-4">
                  <button onClick={handleCopyCodes} className="flex-1 py-3 bg-(--card-bg) border border-(--card-border) text-(--text) rounded-xl text-[11px] font-bold hover:opacity-80 transition-all flex items-center justify-center gap-2">
                    <Copy className="w-3.5 h-3.5" /> Salin
                  </button>
                  <button
                    onClick={handleDownloadCodes}
                    className="flex-1 py-3 bg-(--card-bg) border border-(--card-border) text-(--text) rounded-xl text-[11px] font-bold hover:opacity-80 transition-all flex items-center justify-center gap-2"
                  >
