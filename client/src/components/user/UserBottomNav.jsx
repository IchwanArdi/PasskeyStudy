...
const UserBottomNav = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-(--bg-raised) border-t border-(--border) pb-safe pt-1 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
      <div className="flex items-stretch max-w-lg mx-auto px-2">
        {navItems.map(({ to, icon: Icon, label }) => {
          ...
          return (
            <Link key={to} to={to} className={`flex-1 flex flex-col items-center justify-center py-2 md:py-3 gap-1.5 transition-all relative ${isActive ? 'text-emerald-400' : 'text-gray-500 hover:text-gray-300'}`}>
              ...
              {isActive && <span className="absolute -top-1 left-1/2 -translate-x-1/2 h-1 w-1 bg-emerald-400 rounded-full shadow-[0_0_8px_rgba(52,211,153,0.8)]" />}
            </Link>
          );
        })}
      </div>
