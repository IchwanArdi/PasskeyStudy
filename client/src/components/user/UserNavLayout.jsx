...
const UserNavLayout = ({ children }) => {
  ...
  const hideNav = ['/', '/login', '/register', '/recovery'].includes(location.pathname) || location.pathname.startsWith('/admin') || (location.pathname === '/panduan' && !isAuthenticated());
    ...
    <div className="min-h-screen bg-(--bg)">
      ...
      <div className="hidden md:flex">
        ...
        <main className="flex-1 ml-64 min-h-screen relative overflow-hidden">
          ...
          <div className="absolute top-0 right-0 w-125 h-125 bg-emerald-500/5 rounded-full blur-[120px] -mr-64 -mt-64" />
          <div className="absolute bottom-0 left-0 w-100 h-100 bg-blue-500/5 rounded-full blur-[100px] -ml-32 -mb-32" />

          {/* Konten Halaman */}
          <div className="relative max-w-6xl mx-auto p-8 lg:p-12">{children}</div>
        </main>
      </div>
