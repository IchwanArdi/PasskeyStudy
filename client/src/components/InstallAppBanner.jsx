  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-100 animate-slide-down">
      <div className="mx-auto max-w-lg px-3 pt-3">
        <div className="relative flex items-center gap-3 bg-linear-to-r from-emerald-600/95 to-teal-600/95 backdrop-blur-xl rounded-2xl px-4 py-3 shadow-2xl shadow-emerald-900/30 border border-emerald-400/20">
          {/* Ikon Unduh */}
          ...
          <div className="flex-1 min-w-0">
            <p className="text-white text-sm font-bold leading-tight">Install Aplikasi</p>
            <p className="text-emerald-100/80 text-xs leading-tight mt-0.5">Akses lebih cepat langsung dari layar utama</p>
          </div>

          {/* Tombol Aksi Instalasi */}
          <button onClick={handleInstall} disabled={installing} className="shrink-0 px-4 py-2 bg-white text-emerald-700 rounded-xl text-xs font-bold shadow-lg hover:bg-emerald-50 active:scale-95 transition-all disabled:opacity-60">
            {installing ? 'Menginstal...' : 'Install'}
          </button>

          {/* Tombol Tutup Banner */}
          <button onClick={handleDismiss} className="shrink-0 p-1 text-white/60 hover:text-white transition-colors" aria-label="Tutup">
            <X className="w-4 h-4" />
          </button>
        </div>
      </div>
