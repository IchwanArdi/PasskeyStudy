...
// Item navigasi untuk bilah bawah admin (tampilan mobile)
const navItems = [
...
const AdminBottomNav = () => {
  ...
  // Hanya tampilkan navigasi ini jika pengguna berada di area /admin
  if (!location.pathname.startsWith('/admin')) return null;

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-(--bg)/80 backdrop-blur-2xl border-t border-(--card-border) pb-safe pt-1">
      <div className="flex items-stretch max-w-lg mx-auto px-2">
        {navItems.map(({ to, icon: Icon, label }) => {
          ...
          return (
            <Link key={to} to={to} className={`flex-1 flex flex-col items-center justify-center py-2 md:py-3 gap-1.5 transition-all relative ${isActive ? 'text-red-400' : 'text-gray-500 hover:text-gray-300'}`}>
              ...
              {isActive && <span className="absolute -top-1 left-1/2 -translate-x-1/2 h-1 w-1 bg-red-400 rounded-full shadow-[0_0_8px_rgba(248,113,113,0.8)]" />}
            </Link>
          );
        })}
      </div>
